...
export async function requestWithdrawV4(
  ...
  // Check if a withdraw request already exists to make this idempotent
  // Important for Devnet where previous simulate failures might leave PDAs open
  const existingRequestInfo = await connection.getAccountInfo(withdrawRequestPDA, 'confirmed');
  if (existingRequestInfo) {
    console.log('Withdraw request already exists, skipping request initialization.');
