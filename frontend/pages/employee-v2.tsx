...
export default function EmployeePage() {
  if (!LEGACY_V2_ENABLED) {
    return (
      ...
      >
        <Head>
          <title>Expensee Employee v2 | Disabled</title>
        </Head>
        <section className="panel-card">
          <p className="text-sm text-gray-700">
            Legacy v2 is disabled. Use the v4 employee portal at{' '}
  ...
  const scanPayouts = useCallback(async () => {
    if (wallet.publicKey && employerWallet) {
      try {
        ...
        const parsedIndex = Number(streamIndexInput);
        if (!Number.isFinite(parsedIndex)) return;
        const found = await getPendingPayoutsForWorker(connection, biz, parsedIndex);
        setPendingPayouts(found);
      } catch { /* ignore auto-scan errors */ }
    }
  }, [connection, employerWallet, streamIndexInput, wallet.publicKey]);
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 rounded-2xl border border-amber-300/40 bg-amber-100/40 px-5 py-4 text-sm text-amber-900">
        <div className="font-semibold">Legacy v2 portal (deprecated)</div>
        <div className="mt-1">
          v4 is the default. Use the v4 employee portal at{' '}
          <Link href="/employee" className="underline font-semibold">
            /employee
          </Link>
          .
