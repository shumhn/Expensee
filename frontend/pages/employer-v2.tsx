...
const LEGACY_V2_ENABLED = (process.env.NEXT_PUBLIC_ENABLE_LEGACY_V2 || "").toLowerCase() === "true";
...
export default function EmployerPage() {
    ...
    useCallback(async (): Promise<AgentExecutionStep | null> => {
      ...
      try {
        ...
        if (nextStep.key === "refresh-state") {
          ...
          result = { txid: json.tx, detail: "Minted 1,000 test USDC." };
          ...
          result = { txid, detail: `Deposited ${amount} USDC into payroll wallet.` };
          ...
          result = { txid, detail: `Recovered ${amount} USDC from vault.` };
        }
  ...
  if (!LEGACY_V2_ENABLED) {
    return (
      <PageShell
        icon=""
        title="Expensee"
        subtitle="Employer v2 (disabled)"
        navItems={[
      ...
      >
        <Head>
          <title>Expensee Employer v2 | Disabled</title>
        </Head>
        <section className="panel-card">
          <p className="text-sm text-gray-700">
            Legacy v2 is disabled. Use the v4 employer console at{" "}
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 rounded-2xl border border-amber-300/40 bg-amber-100/40 px-5 py-4 text-sm text-amber-900">
        <div className="font-semibold">Legacy v2 console (deprecated)</div>
        <div className="mt-1">
          v4 is the default. Use the v4 employer console at{' '}
          <Link href="/employer" className="underline font-semibold">
            /employer
          </Link>
          .
          ...
          <section className="hero-card setup-hero glass border-indigo-500/20 shadow-2xl transition-all duration-500">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-xs text-amber-700/80 leading-relaxed">
                        Your vault was initialized with a different token mint than the platform standard (<code className="text-[10px] font-mono bg-amber-100 px-1 py-0.5 rounded">USDC</code>).
            ...
            <AdvancedDetails title="Advanced details">
              <div className="grid gap-1 text-sm">
                <div>USDC mint: {PAYUSD_MINT.toBase58()}</div>
                {derived ? (
                  <>
                    <div>
                  ...
                  <div>
                      <button
                        ...
                        onClick={() =>
                          ...
                          const json = await resp.json();
                            if (!resp.ok || !json?.ok) throw new Error(json?.error || 'Faucet failed');
                            return `Minted 1,000 USDC! tx: ${json.tx}`;
                          })
                        }
